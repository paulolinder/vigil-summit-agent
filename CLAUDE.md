...
**Emails HTML (identidade Vigil):** o corpo dos emails é renderizado em HTML por `app/services/email_templates.py` — um shell único (`_shell`: header navy "Vigil Summit", card branco, footer com assinatura, CSS inline + tabelas Outlook-safe) + 15 miolos em `TEMPLATE_BODIES_HTML` que reusam os MESMOS `{placeholders}` dos textos em `resend_service.TEMPLATES` (paridade validada por `test_all_17_templates_render`). `send_email` envia `html` E `text` (fallback de entregabilidade); `messages.body` guarda o TEXTO (dashboard limpo). Todo valor de `ctx` passa por `_esc` (`html.escape`) antes de entrar no HTML — defesa de injeção via nome/empresa/custom_note. CTA contextual por template (`CTA_MAP`): pré-evento → landing (`FRONTEND_URL`), pós-evento/no-show → link Cal.com (`cta_url`), com fallback landing se ausente/inválido (só `http(s)://`). `render_html` é best-effort: se falhar, `send_email` loga e envia só texto (nunca bloqueia o envio). MVP: o link do Cal.com vai no `custom_note` (texto); o botão "Agendar" cai no fallback landing até o wire estruturado agente→botão. Mudança de template HTML exige redeploy do Railway. NUNCA refatorar removendo o `_esc` do `ctx` (regressão de segurança).

**Confirmação de presença por clique:** o botão dos emails de convite (`welcome`, `confirmation_request`, `confirmation_followup` — set `_CONFIRM_INVITES` em `email_templates.py`) aponta para `{FRONTEND_URL}/confirmar?token=<hmac>`. A página `/confirmar` faz POST em `/api/leads/confirm` (PÚBLICO, sem X-API-Key — o token HMAC stateless de `app/utils/tokens.py` é a credencial), que move `ENRICHED→CONFIRMED` via `atomic_transition_lead_stage` (origem `["ENRICHED"]`) e dispara `run_agent(LEAD_CONFIRMED)` → email `confirmation_agenda` imediato. **Idempotente:** `ALREADY_SET` NÃO redispara o agente; `INVALID_TRANSITION` retorna 200 (UX de email público, não 409 como checkin/no-show operacionais). Token usa `CONFIRM_TOKEN_SECRET` (fallback `API_KEY`) e é **fail-closed** — secret vazio rejeita todos os tokens (nunca valida HMAC de chave vazia). Trocar a secret invalida links em trânsito. NÃO reusar o template `agenda` para esse disparo (ele tem âncoras "3 dias/sábado" fixas) — usar `confirmation_agenda`.
